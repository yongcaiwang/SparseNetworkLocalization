...
% vmeans 2*n(顶点数) 顶点的初始位置
% eids  2*m(边数) 边的起点终点编号
% emeans 2*m 各个边的测量值；第一行为测距；第二行为测量角度；0表示没有测量
% einfs 2*2*m: 各个边的信息矩阵；
% etype m*1,各个边的类型：1(仅测距的边); 2(仅测角度的边); 3(同时具有距离和角度测量的边)
% n 迭代次数
...
xj=v_j(1);yj=v_j(2);
z_ij = emeans(:,k);
temp_d=sqrt((xi-xj)^2+(yi-yj)^2);
temp_type=etype(k);
if temp_type==1%只有距离约束
    %e=(v_j-v_i)-(v_j-v_i)*(z_ij(1)/temp_d);
    %A=[v_i(1)-v_j(1),v_i(2)-v_j(2);0,0]/temp_d;
    e= z_ij(1)-temp_d;   
    A=[-(xi-xj)/temp_d, -(yi-yj)/temp_d];
    %A= [-1,0;0, -1 ];
    B=-A;
